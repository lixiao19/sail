...
PacketTagList::Add (const Tag &tag) const
{
  ...
  // Some simulation paths overwrite routing metadata (for example FlowIdTag)
  // on packet copies that may already carry the same tag kind. Treat this as a
  // replace instead of aborting the whole simulation.
  for (struct TagData *cur = m_next; cur != 0; cur = cur->next)
    {
      if (cur->tid == tag.GetInstanceTypeId ())
        {
          Tag &mutableTag = const_cast<Tag &> (tag);
          const_cast<PacketTagList *> (this)->Replace (mutableTag);
          return;
        }
    }
