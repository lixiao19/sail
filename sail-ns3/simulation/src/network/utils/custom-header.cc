...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  // L3
  int l3Size = 0;
  uint8_t shimProt = 0;  // ★ 新增:保存 IPv4 Protocol 字段
  int wanOptSize = 0;    // ★ 新增:WanOptHeader 的大小
  if (headerType & L3_Header){
	  i = start;
	  i.Next(l2Size);
  ...
  if (shimProt == 0xFA) {
	  // IPv4 Protocol 字段是 0xFA,表示下一个是 WanOptHeader
	
	  uint8_t flags = i.ReadU8();
	  wanOptInfo.isAggregated     = (flags & 0x01);
	  wanOptInfo.isDigestPacket   = (flags & 0x02) >> 1;
	  wanOptInfo.isFakePacket     = (flags & 0x04) >> 2;
	  wanOptInfo.isRepeatPacket = (flags & 0x08) >> 3;
	
	  // 读取真正的下一层协议(例如 UDP=17)
	  l3Prot = i.ReadU8();
	  ...
	  // 读取新的 PSN
	  wanOptInfo.newPsn = i.ReadNtohU64();
	
	  wanOptSize = 12;  // WanOptHeader 总共 12 字节
	  // 现在 i 已经指向真正的 L4 Header (例如 UDP)
  } else {
	  // 没有 WanOptHeader,直接用 shimProt 作为 L4 协议号
	  l3Prot = shimProt;
  }
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  i = start;
		  i.Next(l2Size + l3Size + wanOptSize);
