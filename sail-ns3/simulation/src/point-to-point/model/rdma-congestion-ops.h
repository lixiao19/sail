...
class RdmaCongestionOps : public Object {
 public:
  ...
  virtual void HandleAck(
      ...
      CustomHeader& ch) = 0;
  /**
   * \brief This function is called when rdma-hw receives an explicit CNP packet.
   */
  virtual void HandleCnp(Ptr<RdmaQueuePair> qp);
  /**
...
class HpccEcn : public RdmaCongestionOps {
 public:
  ...
  virtual void HandleAck(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader& ch)
      override;
  virtual void HandleCnp(Ptr<RdmaQueuePair> qp) override;
  std::string GetName() const override {
    return "HPCC-ECN";
  }
  virtual void RampUp();
