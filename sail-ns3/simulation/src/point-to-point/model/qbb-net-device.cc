...
void QbbNetDevice::Receive(Ptr<Packet> packet) {
  ...
  if (ch.l3Prot == 0xFE) { // PFC
    ...
    if (m_node->GetNodeType() > 0 && ch.m_tos != 4 &&
      ...
      // sid << std::endl;
      // Some in-network recovery paths may forward an internal packet copy that
      // still carries a stale FlowIdTag. Overwrite it instead of asserting on a
      // duplicate tag.
      FlowIdTag flowIdTag(m_ifIndex);
...
bool QbbNetDevice::TransmitStart(Ptr<Packet> p) {
  ...
  // Schedule the send-complete callback after this packet's transmission time.
...
void QbbNetDevice::TakeDown() {
  // Drain device queues before marking the link down.
  if (m_node->GetNodeType() == 0) {
    ...
    // Switch forwarding-table updates are handled outside this device method.
  }
  m_linkUp = false;
}
