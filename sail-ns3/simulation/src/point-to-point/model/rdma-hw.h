...
class RdmaHw : public Object {
 public:
  ...
  bool m_rateBound;
  bool m_sr_enabled; // whether SR is enabled
  uint32_t m_reliability_mode;
  uint32_t m_irn_buffer_slots;
  bool m_bdp_fc_enabled;
  uint32_t m_bdp_fc_window_slots;
  ...
  ReliabilityMode ResolveReliabilityMode() const;
  bool UseSelectiveRepeat() const;
  bool IsIrnMode() const;
  bool UseBdpFcWindow() const;
  bool IsLr2Mode() const;
  bool IsSailMode() const;
  uint32_t GetSelectiveBufferSlots() const;
  uint64_t GetGbnSequenceNakSeq(uint64_t expected) const;
  bool TryEnterGbnSequenceNakState(Ptr<RdmaRxQueuePair> rxQp);
  void ClearGbnSequenceNakState(Ptr<RdmaRxQueuePair> rxQp, uint64_t oldExpected);
  bool AdvanceSailCommit(Ptr<RdmaRxQueuePair> rxQp, uint64_t* commitSeqOut = nullptr);
  void InsertSailOutstandingLoss(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  void EraseSailOutstandingLoss(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  void ReissueSailOutstandingLoss(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  bool HandleSailOutstandingLossArrival(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  void ClearResolvedSailLosses(Ptr<RdmaRxQueuePair> rxQp, uint64_t commitSeq);
  void SendSailSwitchNackOnce(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  void SendSailSwitchNack(Ptr<RdmaRxQueuePair> rxQp, uint64_t seq);
  bool ShouldSuppressSailSwitchNack(Ptr<RdmaQueuePair> qp, uint64_t seq);
  bool HasOutstandingSailCommit(Ptr<RdmaQueuePair> qp) const;
  void HandleSailLossRetry(
      uint32_t senderIp,
      uint32_t receiverIp,
      uint16_t senderPort,
      ...
      uint16_t pg,
      uint64_t seq);
  int ReceiveSailLossReport(Ptr<Packet> p, CustomHeader& ch);
  int HandleSailReceive(
      Ptr<Packet> p,
      CustomHeader& ch,
      Ptr<RdmaRxQueuePair> rxQp,
      uint32_t payloadSize,
      bool isFakePacket);
  void RestartRtoTimer(Ptr<RdmaQueuePair> qp);
  void RetransmitFromSeq(Ptr<RdmaQueuePair> qp, uint64_t seq);
  void StartLr2SwitchRecovery(Ptr<RdmaQueuePair> qp, uint64_t seq);
  void AddHeader(Ptr<Packet> p, uint16_t protocolNumber);
  static uint16_t EtherToPpp(uint16_t protocol);

  void RecoverQueue(Ptr<RdmaQueuePair> qp);
  void RecoverQueue(Ptr<RdmaQueuePair> qp, uint64_t seq, bool updateAck);
  ...
  Ptr<Packet> GenRepeatPacket(Ptr<RdmaQueuePair> qp, uint32_t pkt_size, uint32_t seq);
  int SendAck(
      Ptr<RdmaRxQueuePair> rxQp,
      CustomHeader& ch,
      int x,
  ...
  void HandleRtoTimeout(Ptr<RdmaQueuePair> qp);
  /******************************
  ...
  Time m_rto; //< Requester retransmission timeout
  Time m_sailLossRetryInterval; //< Receiver-side SAIL switch-NACK retry timeout
  static const uint32_t kSailMaxLossRetries = 100;
  const uint32_t m_sailSwitchNackCopies = 2;
  const Time m_sailSwitchNackRedundancyGap = MicroSeconds(100);
  const Time m_sailSwitchNackSuppressInterval = MicroSeconds(500);
};
