...
uint32_t stop_on_foreground_only = 0;
uint32_t stop_flow_dport = 100;
uint32_t stop_flow_count = 0;
std::string collective_mode = "none";
std::string collective_plan_file = "";
double collective_stop_delay_ms = 35.0;
...
void InitConfigMap() {
  ...
  config_map["STOP_ON_FOREGROUND_ONLY"] =
      std::make_unique<ConfigVar<uint32_t>>(stop_on_foreground_only);
  config_map["STOP_FLOW_DPORT"] =
      std::make_unique<ConfigVar<uint32_t>>(stop_flow_dport);
  config_map["STOP_FLOW_COUNT"] =
      std::make_unique<ConfigVar<uint32_t>>(stop_flow_count);
  config_map["COLLECTIVE_MODE"] =
      std::make_unique<ConfigVar<std::string>>(collective_mode);
  config_map["COLLECTIVE_PLAN_FILE"] =
      std::make_unique<ConfigVar<std::string>>(collective_plan_file);
  config_map["COLLECTIVE_STOP_DELAY_MS"] =
      std::make_unique<ConfigVar<double>>(collective_stop_delay_ms);
...
void SetupNetwork(
  ...
  bool collective_enabled = collective_mode == "allreduce";
  topof.open(topology_file.c_str());
  if (!collective_enabled) {
    flowf.open(flow_file.c_str());
  }
  tracef.open(trace_file.c_str());
  if (!topof.is_open() || (!collective_enabled && !flowf.is_open()) || !tracef.is_open()) {
    ...
    if (n.Get(i)->GetNodeType() == 1) { // is switch
      ...
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        ...
        uint64_t delay = DynamicCast<QbbChannel>(dev->GetChannel())
                             ->GetDelay()
                             .GetTimeStep();
        uint64_t pfc_delay_cutoff = static_cast<uint64_t>(MilliSeconds(1).GetTimeStep());
        bool port_pfc_enabled = delay < pfc_delay_cutoff;
        // uint32_t headroom = 150000; // rate * delay / 8 / 1000000000 * 3;
        uint32_t headroom = 0;
        if (port_pfc_enabled)
          headroom = rate * delay / 8 / 1000000000 * 2 + payload * 2; // BDP + 2 packet
        sw->m_mmu->ConfigHdrm(j, headroom);
        sw->m_mmu->ConfigPfc(j, port_pfc_enabled);
